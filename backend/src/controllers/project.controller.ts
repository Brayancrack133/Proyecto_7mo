...
export class ProjectController {
  ...
  static async create(req: Request, res: Response) {
    try {
      const {
        ...
        id_jefe,
        id_estado_proyecto,
      } = req.body; // Validaciones (sin cambios, son correctas)
  ...
  static async update(req: Request, res: Response) {
    try {
      const id = parseInt(req.params.id); // CORRECCIÓN DE SEGURIDAD: Obtener userId del token (res.locals), NO del body.
      ...
      const { nombre, descripcion, fecha_inicio, fecha_fin, id_jefe } =
        req.body; // Validar fecha
  ...
  static async getByJefe(req: Request, res: Response) {
    try {
      ...
      if (isNaN(jefeId)) {
        return res
          .status(400)
          .json({ success: false, message: "ID de jefe inválido o faltante" });
      }
  ...
  static async getByColaborador(req: Request, res: Response) {
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Error desconocido";
      console.error(
        "Error al obtener proyectos del colaborador:",
        errorMessage
      );
      res
        .status(500)
  ...
  static async changeStatus(req: Request, res: Response) {
    try {
      const id = parseInt(req.params.id);
      const { id_estado_proyecto } = req.body; // CORRECCIÓN DE SEGURIDAD: Obtener userId del token (res.locals), NO del body.
      const userId = res.locals.user?.id || 1; // Asume que el middleware lo inyecta
      ...
      const updated = await ProjectModel.changeStatus(
        id,
        id_estado_proyecto,
        userId
      );
      ...
      if (!updated) {
        return res
          .status(404)
          .json({ success: false, message: "No se pudo actualizar el estado" });
      }
  ...
  static async addTeamMember(req: Request, res: Response) {
    try {
      ...
      const { id_usuario, rol_en_equipo } = req.body; // CORRECCIÓN DE SEGURIDAD: Obtener adminId del token (res.locals), NO del body.
